...
def solve_lambda(theta_deg, r):
    ...
    def f(lam):
        alpha_deg = np.rad2deg(theta_rad +  np.exp(-1*(r+0.5)**6)*0.06981 - lam/r + 0.052)
...
def compute_CT_CP(theta_deg, r_vals):
    ...
    phi = lam_r / r_vals
    alpha_eff = np.rad2deg(np.deg2rad(theta_deg) +  np.exp(-1*(r_vals+0.5)**6)*0.06981 - phi + 0.052 )
    ...
    omega = (0.08*9.81/(1.225*(0.08)**4*np.pi*CT))**0.5
    P = CP*1.225*np.pi*(0.08)**5*(omega**3)
    time = 3.7*3600*1/(P*60)
    CP_corr = CP
    FM = ((CT)**1.5)/(CP*1.414)
    return CT, CP, CP_corr , FM , time 
...
for th in thetas:
    CT, CP, CP_corr ,FM , time = compute_CT_CP(th, r_vals)
    CT_list.append(CT)
    CP_list.append(CP)
